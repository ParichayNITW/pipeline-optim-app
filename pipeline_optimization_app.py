...
st.markdown(
    """
    <div style='background: linear-gradient(90deg, #4f81bd, #2c3e50);
...
with st.sidebar:
    ...
    with st.expander("Adjust Parameters", expanded=True):
        ...
        neos_email = st.text_input("NEOS Email (for remote solve)", value="")
...
if run:
    if not neos_email:
        st.error("⚠️ Please enter your NEOS-registered email address in the sidebar.")
        st.stop()

    os.environ["NEOS_EMAIL"] = neos_email
    with st.spinner("Submitting to NEOS… this can take 2–3 minutes"):
        try:
    ...
    with tab1:
        ...
        st.dataframe(df.style.format(formatters).set_properties(**{"text-align": "center"}), use_container_width=True)
    ...
    with tab2:
        ...
        fig = px.bar(cost_df.melt(id_vars="Station", value_vars=["Power & Fuel", "DRA"]),
    ...
    with tab3:
        ...
        perf_df = pd.DataFrame({
            ...
            "Pump Speed":      [res.get(f"speed_{s.lower()}", 0)      for s in stations],
            "Pump Efficiency": [res.get(f"efficiency_{s.lower()}", 0) for s in stations],
        })
        fig2 = go.Figure()
        fig2.add_trace(go.Bar(x=perf_df["Station"], y=perf_df["Head Loss"], name="Head Loss (mcl)"))
        fig2.add_trace(go.Scatter(x=perf_df["Station"], y=perf_df["Pump Speed"], name="Pump Speed (rpm)",
                                  yaxis="y2", mode="lines+markers"))
        fig2.add_trace(go.Scatter(x=perf_df["Station"], y=perf_df["Pump Efficiency"], name="Pump Efficiency (%)",
                                  yaxis="y2", mode="lines+markers"))
        fig2.update_layout(
            yaxis2=dict(overlaying="y", side="right", title="Speed / Efficiency"),
            xaxis_title="Station", height=500
        )
